...
def plot_volume(vol):
    ...
    plt.cla()
    scat = ax.scatter(x.flatten(), y.flatten(), z.flatten(), c=vol.reshape(-1), cmap='binary',
...
if len(data_in.shape) == 4:
    ani = animation.FuncAnimation(fig, update_plot, interval=args.time_step, frames=data_in.shape[3])
    plt.show()
